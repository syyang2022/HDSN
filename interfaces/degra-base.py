...
from model import tbsrn, tsrn, edsr, srcnn, srresnet, crnn,bicubic
import dataset.dataset as dataset
#from dataset import lmdbDataset, alignCollate_real, ConcatDataset, lmdbDataset_real, alignCollate_syn, lmdbDataset_mix
from dataset import lmdbDataset, alignCollate_real, ConcatDataset, lmdbDataset_real, alignCollate_syn, lmdbDataset_mix,lmdbDataset_real2
...
class TextBase(object):
    def __init__(self, config, args):
        ...
        if self.args.syn:
            ...
            self.load_dataset2 = lmdbDataset_real2
    ...
    def get_val_data(self):
        ...
        for data_dir_ in cfg.VAL.val_data_dir:
            val_dataset, val_loader = self.get_test_data_2(data_dir_)
    ...
    def get_test_data_2(self, dir_):
        cfg = self.config.TRAIN
        test_dataset = self.load_dataset2(root=dir_,
                                         voc_type=cfg.voc_type,
                                         max_len=cfg.max_len,
                                         test=True,
                                         )
        ...
        if self.args.arch != 'bicubic':
            ...
            if self.resume is not '':
                ...
                if self.config.TRAIN.ngpu == 1:
                    model.load_state_dict(torch.load(self.resume)['state_dict_G'],False)
                else:
                    model.load_state_dict(
                        {'module.' + k: v for k, v in torch.load(self.resume)['state_dict_G'].items()},False)
