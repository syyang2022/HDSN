...
class TSRN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.stn and self.training:
            ...
            # 将 tensor 调整为图片格式并保存
            #save_image(x_single, f'/mnt/sdb/xlf/FudanOCR-main2/scene-text-telescope/img_sr/i.png', normalize=True, range=(-1, 1))
            #np_array = x.detach().cpu().numpy()
            #np_array = np_array[0]
            #tran=transforms.ToTensor()
            #image = Image.fromarray(np_array.astype(np.uint8), mode='RGB')
            #save_image(tran(image), f'/mnt/sdb/xlf/FudanOCR-main2/scene-text-telescope/img_sr/i.png')
            #print(type(x)) tensor
            #print(type(ctrl_points_x)) tensor
            #print("x.shape")
            #print(x.size())
            #print("ctrl.shape")
            #print(ctrl_points_x.size())
